...
import platform
isWin64 = platform.architecture()[0] == '64bit' and platform.architecture()[1] == 'WindowsPE'
...
def withCHID(fcn):
    ...
    def wrapper(*args, **kwds):
        ...
        if len(args)>0:
            chid = args[0]
            args = list(args)
            if isinstance(chid, int) or isinstance(chid, long):
        ...
        if len(args)>0:
            ...
            if not isConnected(chid):
                ...
                if not connect_channel(chid, timeout=timeout):
                    raise ChannelAccessException(fmt % (fcn.__name__, timeout))
...
if isWin64:
    # For Win64 the compiler will convert all arguments over 8 bytes into a pointer
    # ctypes currently does not take this into account, so we need to take care of it ourselves
    # by using a ctypes pointer
    _CB_CONNECT = ctypes.CFUNCTYPE(None, ctypes.POINTER(dbr.connection_args))(_onConnectionEvent)
    _CB_PUTWAIT = ctypes.CFUNCTYPE(None, ctypes.POINTER(dbr.event_handler_args))(_onPutEvent)
    _CB_GET     = ctypes.CFUNCTYPE(None, ctypes.POINTER(dbr.event_handler_args))(_onGetEvent)
    _CB_EVENT   = ctypes.CFUNCTYPE(None, ctypes.POINTER(dbr.event_handler_args))(_onMonitorEvent)
else:
